...
p.add_argument('--theta_max', type=float, default=1.1, required=False, help='Turn rate of the car')
p.add_argument('--angle_alpha', type=float, default=1.0, required=False, help='Angle alpha coefficient.')
p.add_argument('--targetR', type=float, default=0.75, required=False, help='Collision radisu between vehicles')
...
dataset = dataio.ReachabilityParticle2DSource(numpoints=65000, targetR=opt.targetR, velocity=opt.velocity, 
                                          theta_max=opt.theta_max, pretrain=opt.pretrain, tMin=opt.tMin,
...
# Define the loss
loss_fn = loss_functions.initialize_hj_particle2D(dataset, opt.minWith)
